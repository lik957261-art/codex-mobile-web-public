...
function createThreadListRuntime(deps = {}) {
	...
	const THREAD_LIST_CACHE_MAX_AGE_MS = 10080 * 60 * 1e3;
	...
	function cachedThreadSummary(thread) {
		...
		const summary = {};
		for (const field of THREAD_LIST_CACHE_FIELDS) if (Object.prototype.hasOwnProperty.call(source, field)) summary[field] = source[field];
		try {
	...
	function readThreadListCache() {
		...
		try {
			...
			if (!payload || payload.version !== THREAD_LIST_CACHE_VERSION || !savedAt || ageMs < -3e5 || ageMs > THREAD_LIST_CACHE_MAX_AGE_MS) {
				localStorage.removeItem(THREAD_LIST_CACHE_STORAGE_KEY);
				return [];
			}
			return (Array.isArray(payload.threads) ? payload.threads : []).map(cachedThreadSummary).filter(Boolean).slice(0, THREAD_LIST_CACHE_MAX_THREADS);
		} catch (_) {
			try {
				localStorage.removeItem(THREAD_LIST_CACHE_STORAGE_KEY);
			} catch (_) {}
	...
	function persistThreadListCache(threads = state.threads) {
		...
		const summaries = visibleThreads(Array.isArray(threads) ? threads : []).map(cachedThreadSummary).filter(Boolean).slice(0, THREAD_LIST_CACHE_MAX_THREADS);
		try {
	...
	async function loadWorkspaces(options = {}) {
		...
		const requestOptions = Number(options.timeoutMs) > 0 ? { timeoutMs: Number(options.timeoutMs) } : void 0;
