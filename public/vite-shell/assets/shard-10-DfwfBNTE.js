...
async function bootstrap() {
	...
	loadWorkspaces({ timeoutMs: 8e3 }).then(() => {
		...
		setTimeout(() => {
			...
			loadWorkspaces({ timeoutMs: 12e3 }).catch(() => {});
		}, 3500);
	});
	const statusStartedAt = nowPerfMs();
	const status = await api("/api/status", { timeoutMs: 8e3 }).catch((err) => {
	...
	await loadThreadDisplaySettings({
		render: false,
		timeoutMs: 3e3
	}).catch((err) => {
