...
test("superseded live turns keep uploaded image user messages while hiding stale text steering", () => {
  const { visibleItemsForTurn } = evaluatedVisibleItemsForTurn();
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-20/thread-id/1781938485528-homeai-upload-080D5F0E.jpg";
  const turn = {
    status: { type: "completed", mobileSupersededLive: true, previousType: "inProgress" },
    items: [
...
test("Hermes proxy app-server text upload summaries render png thumbnails without exposing local paths", () => {
  ...
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-28/019eee6c-a6f5-7b20-bfb4-f96ccb6431b3/1782622603833-d5ac6a1e9f2e-homeai-upload-174E84FB-38DA-4D53-AA6F-9FC6E3CA289C.png";
  const html = renderInputContent([
    {
      type: "text",
...
test("protected upload image errors are probed before showing failed fallback", async () => {
  ...
  const toggledClasses = [];
  const attributes = {};
  let imageSrc = "/api/uploads/file?path=%2FUsers%2Fexample%2F.codex-mobile-web%2Fuploads%2Fthread%2Fhomeai-upload.jpg&key=stale";
  const figure = {
    classList: {
      add(value) {
        addedClasses.push(value);
...
test("raw app-server input image url local upload paths use authenticated uploads route", () => {
  ...
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-08/thread-id/1780936946968-IMG_5882.jpg";
  const html = renderInputContent([
    {
      type: "input_text",
...
test("durable upload summaries take precedence over stale blob image parts", () => {
  const renderInputContent = evaluatedInputContentRendererWithKey("session-key");
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-20/thread-id/1781947353793-homeai-upload-2B62320E.jpg";
  const html = renderInputContent([
    {
      type: "input_text",
  ...
  assert.equal(userMessagesLikelySame(
    ...
    },
    {
      ...
      content: [
        {
          type: "input_text",
          text: "Uploaded attachments:\n- IMG_5882.jpg (image, image/jpeg, 101.7 KB): /Users/example/.codex-mobile-web/uploads/thread/IMG_5882.jpg",
        },
        { type: "localImage", path: "/Users/example/.codex-mobile-web/uploads/thread/IMG_5882.jpg" },
      ],
    },
  ), true);
});
...
test("durable uploaded image messages replace revoked optimistic blob previews", () => {
  const mergeItemsPreservingLocalVisible = evaluatedMergeItemsPreservingLocalVisibleWithRealVisibleWeight();
  const renderInputContent = evaluatedInputContentRendererWithKey("test-key");
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-20/thread/homeai-upload-54FE12C6.jpg";
  const localImage = {
...
test("cross-turn normalization drops later optimistic upload image echoes after durable image appears", () => {
  const normalizeThreadVisibleUserMessages = evaluatedNormalizeThreadVisibleUserMessages();
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-20/thread/1781940095858-homeai-upload-49DBCECD.jpg";
  const thread = {
    turns: [
      {
        id: "durable-turn",
...
test("v4 projection merge does not preserve server-suppressed upload image echoes", () => {
  const mergeThreadPreservingVisibleItems = evaluatedMergeThreadPreservingVisibleItems();
  const uploadPath = "/Users/example/.codex-mobile-web/uploads/2026-06-23/thread/1782217099872-Screenshot_20260623_201803_Home AI.jpg";
  const existingThread = {
    id: "thread-new",
    mobileProjectionVersion: "v4",
