...
test("core public config route responds without waiting for quota refresh", async () => {
...
test("core shared-chain restart route forwards bounded default shell mode", async () => {
  ...
  let sent = null;
  const service = createCoreApiRouteService({
    activeProfileRestartOptions: () => ({ profileId: "active", codexHome: "/home/example/active/.codex" }),
  ...
  assert.equal(restartOptions.codexHome, "/home/example/active/.codex");
...
test("core profile switch fails closed when target quota preflight fails", async () => {
        ...
        profiles: [
          {
            id: "target",
            label: "Target",
            codexHome: "/home/example/target/.codex",
  ...
  assert.deepEqual(trustCalls, ["/home/example/target/.codex"]);
  assert.deepEqual(mcpCalls, ["/home/example/target/.codex"]);
});
