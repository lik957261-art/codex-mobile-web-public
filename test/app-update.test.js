...
test("source snapshot update status keeps the deployed version badge neutral", () => {
  ...
  const state = {
    ...
    appUpdateStatus: {
      ...
      error: "legacy server diagnostic",
  ...
  const runtime = createAppUpdateRuntime({
    state,
    CLIENT_BUILD_ID: "0.1.13|codex-mobile-shell-v629-test",
    $: (id) => id === "appUpdateStatus" ? element : null,
  });

  runtime.renderAppUpdateStatus();

  assert.equal(element.textContent, "v0.1.13 · 客户端 v629");
  assert.equal(element.title, "当前为稳定发布快照；源码更新由部署流程管理");
  assert.equal(classes.get("blocked"), false);
  assert.equal(classes.get("available"), false);
});
