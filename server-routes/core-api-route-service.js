...
function createCoreApiRouteService(deps = {}) {
  ...
  let statusRefreshLastStartedAtMs = 0;

  function scheduleStatusBackgroundRefresh() {
    const now = Date.now();
    if (statusRefreshInFlight) return statusRefreshInFlight;
    if (now - statusRefreshLastStartedAtMs < 15000) return null;
    statusRefreshLastStartedAtMs = now;
    statusRefreshInFlight = Promise.resolve()
      .then(async () => {
        await codex.ensure().catch((err) => {
          codex.lastError = err && err.message || String(err);
        });
        await codex.refreshRateLimitsIfMissing().catch((err) => {
          codex.lastError = err && err.message || String(err);
        });
        loadRecentRateLimitsFromRollouts();
      })
      ...
      if (truthyParam(url.searchParams.get("muxMetrics"))) {
        try {
          status.muxMetrics = await Promise.race([
            codex.readMuxMetrics(["thread/list"]),
            new Promise((resolve) => setTimeout(() => resolve({ timeout: true }), 750)),
          ]);
        } catch (err) {
          status.muxMetrics = { error: err && err.message || String(err) };
        }
      }
      sendJson(200, status);
      return { handled: true };
