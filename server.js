...
const MOBILE_DEFAULT_MODEL = normalizeEnumValue(
  process.env.CODEX_MOBILE_DEFAULT_MODEL || "",
  new Set(MODEL_OPTIONS),
) || "";
const MOBILE_DEFAULT_REASONING_EFFORT = normalizeEnumValue(
  process.env.CODEX_MOBILE_DEFAULT_REASONING_EFFORT || "",
...
async function handleApi(req, res) {
  ...
  if (url.pathname === "/api/public-config") {
    ...
    sendJson(res, 200, {
      ...
      defaultModel: MOBILE_DEFAULT_MODEL || CODEX_CONFIG_DEFAULTS.model || DEFAULT_MODEL,
      defaultReasoningEffort: MOBILE_DEFAULT_REASONING_EFFORT || CODEX_CONFIG_DEFAULTS.reasoningEffort,
  ...
  if (url.pathname === "/api/threads/new-message" && req.method === "POST") {
    ...
    try {
      const result = await runMessageSubmissionOnce(submissionKeys, uploads, async () => {
        ...
        if (requestedModel || MOBILE_DEFAULT_MODEL) startParams.model = requestedModel || MOBILE_DEFAULT_MODEL;
        const startResult = await codex.request("thread/start", startParams, {
          timeoutMs: MUTATION_RPC_TIMEOUT_MS,
          retry: false,
        });
        ...
        if (requestedModel || MOBILE_DEFAULT_MODEL) turnParams.model = requestedModel || MOBILE_DEFAULT_MODEL;
        if (requestedEffort || MOBILE_DEFAULT_REASONING_EFFORT) turnParams.effort = requestedEffort || MOBILE_DEFAULT_REASONING_EFFORT;
  ...
  if (messages && req.method === "POST") {
    ...
    try {
      result = await runMessageSubmissionOnce(submissionKeys, uploads, async () => {
        ...
        if (requestedModel || MOBILE_DEFAULT_MODEL) params.model = requestedModel || MOBILE_DEFAULT_MODEL;
        if (requestedEffort || MOBILE_DEFAULT_REASONING_EFFORT) params.effort = requestedEffort || MOBILE_DEFAULT_REASONING_EFFORT;
