...
## 2026-07-11 Public 发布说明（0.1.14 手机公网冷启动优化）

本次补丁针对真实 iPhone 通过公网入口访问时的偶发慢启动：页面外壳和当前
对话已经可用，但非关键的工作区请求可能在移动网络上等待到超时，进而阻塞
侧边栏列表并显示红色错误。

主要更新：

- 手机端先恢复最近最多 40 条对话标题摘要，再在后台刷新实时列表。
- 本地摘要缓存只保留列表显示所需字段，最长保存 7 天，不保存 turn、消息项
  或消息正文。
- 工作区刷新不再阻塞启动；状态、工作区和多窗口显示设置请求增加有界超时，
  工作区失败后只静默重试一次。
- 打开侧边栏时直接刷新对话列表，不再串行等待工作区元数据。
- 后台列表刷新超时时保留当前可用对话和缓存列表，不再用红色错误覆盖正常界面。
- 继续使用 0.1.13 的完整上游源码、Home AI 插件接口和稳定经典界面，不改变
  `8789` 服务端口、`8443` 公网入口或其他服务端口。

## 2026-07-11 Public 发布说明（0.1.13 上游完整同步）

本次发布以本机已验证的 0.1.12 正式版为稳定基线，补回此前被过宽
`runtime/` 忽略规则漏掉的 `services/runtime/` 完整源码，并合并原开发者
`pentiumxp/codex-mobile-web-public` 到 `5f5d8e9` 的最新公开源码沿革。

主要更新：

- 完整加入 Remote Managed Workspace、配对恢复、本地执行桥、节点轮询和
  受限执行凭据源码。
- 加入 `@loop`、角色线程生命周期、任务卡执行权限和终态回传关联。
- 保留 0.1.12 的移动启动、快速登录、触控、长按语音输入、缓存预热和
  macOS 启动器发现优化。
- 同步原生 ESM 前端源码、诊断工具、测试和部署脚本，便于相同配置的新机器
  直接安装部署。
- 前端构建标识升级为 `codex-mobile-shell-v629`，生产默认仍为稳定经典界面。
- 公开仓继续排除访问密钥、配对凭据、运行数据库、日志、上传、输出、备份、
  完整 rollout 和 `.agent-context`。

## 2026-07-11 Public 发布说明（0.1.12 移动启动与便携部署）

本次 Public 发布同步已经在本机生产服务和真实 iPhone 上验证的移动启动、
登录恢复和界面稳定性修复。生产默认仍使用经典界面，Vite shell 只作为已构建、
可核验但不默认启用的后续入口，避免升级时改变现有使用习惯。

主要更新：

- 启动阶段延后非关键工作，预热线程列表和详情缓存，减少慢网络下登录后等待时间。
- 将 server、API route、线程列表、线程详情、任务卡和浏览器运行时拆分为可测试服务，
  保留原有接口和经典界面行为。
- 加强 active turn 恢复、客户端事件诊断、通知、媒体预览和移动端 DOM 稳定性。
- 增加 Vite shell 构建产物、资源清单生成和一致性校验，但不改变生产默认入口。
- macOS 启动器自动发现当前 ChatGPT 内置 Codex 或旧 Codex.app 可执行文件。
- Node 启动路径按随部署 runtime、Homebrew Node 24、常规 Homebrew、PATH 顺序发现，
  避免 LaunchAgent 依赖临时 Codex 缓存目录。
- 公开仓继续排除本机包装器、`.agent-context`、访问 key、运行数据库、完整 rollout、
  日志、上传、输出和备份。

发布前验证范围包括 `npm test`、`npm run check`、`npm run check:macos`、
前端 manifest/build 校验、隐私扫描、Git 差异检查，以及本机和公网移动端烟雾测试。
